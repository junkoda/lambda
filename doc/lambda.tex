...
\paragraph{Derivation of power-spectrum decomposition}
...
\vspace{5mm}
Multipole moments $\xi_\ell(r, \nu)$ and $P_\ell(k, \nu)$ are related though the spherical-Bessel transform:
%
\begin{align}
...
\begin{align}
  ...
  \xi(r, \nu) = \int \! d^3 k \, e^{i\bm{k}\cdot\bm{r}} P(k, \mu).
\end{align}
%
\begin{equation}
  \int\! Y_\ell^m(\theta, \varphi) d \varphi = 0 \mbox{ for $m \neq 0$}
\end{equation}
and the orthognality of the Legendre polynomials give equations~(\ref{eq:spherical-bessel-dr}--\ref{eq:spherical-bessel-dk}).
